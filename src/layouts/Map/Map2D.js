...
componentDidMount() {
    ...
    if (!window.google) {
        ...
        s.src = `https://maps.googleapis.com/maps/api/js?key=AIzaSyCUU90mGUU8I7wHFPfteUJmZHdy-CSOApM&libraries=places&sensor=true`;
...
initMap=() => {
    ...
    self.map = new window.google.maps.Map(document.getElementById("map_canvas"), myOptions);
...
createMarker = (lat, long, title, contentString, i) => {
    ...
    self.markers[i] = new window.google.maps.Marker({
        position: new window.google.maps.LatLng(lat, long),
        map: self.map, 
        title:title
    });

    window.google.maps.event.addListener(self.markers[i], 'click', function() {
...
plotOnMap = (locs) => {
    ...
    for (var i=0;i<locs.length;i++) {
        self.contentString = '<table className={"attrs"}>';
